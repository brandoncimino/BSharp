...
namespace System.Reflection {
    ...
    public sealed class NullabilityInfoContext {
        ...
        private readonly Dictionary<Module, NotAnnotatedStatus_BRANDON> _publicOnlyModules        = new();
        ...
        [Flags]
        private enum NotAnnotatedStatus_BRANDON {
        ...
        private NullabilityState GetNullableContext(MemberInfo? memberInfo) {
            while (memberInfo != null) {
                ...
                foreach (CustomAttributeData attribute in memberInfo.GetCustomAttributesData()) {
                    if (attribute is { AttributeType: { Name: "NullableContextAttribute", Namespace: CompilerServicesNameSpace }, ConstructorArguments.Count: 1 }) {
                ...
                if (attribute.AttributeType.Namespace == "System.Diagnostics.CodeAnalysis") {
                    if (attribute.AttributeType.Name == "NotNullAttribute" &&
                        ...
                        break;
                             ...
                             nullability is { ReadState: NullabilityState.NotNull, Type.IsValueType: false }) {
                        nullability.ReadState = NullabilityState.Nullable;
                        break;
                    }
                    ...
                    if (attribute.AttributeType.Name == "DisallowNullAttribute" &&
                        ...
                        break;
                             ...
                             nullability is { WriteState: NullabilityState.NotNull, Type.IsValueType: false }) {
                        nullability.WriteState = NullabilityState.Nullable;
                        break;
                    }
                }
        ...
        private bool IsPublicOnly(bool isPrivate, bool isFamilyAndAssembly, bool isAssembly, Module module) {
            if (!_publicOnlyModules.TryGetValue(module, out NotAnnotatedStatus_BRANDON value)) {
                value = PopulateAnnotationInfo(module.GetCustomAttributesData());
                _publicOnlyModules.Add(module, value);
            }

            if (value == NotAnnotatedStatus_BRANDON.None) {
                return false;
            }

            if ((isPrivate || isFamilyAndAssembly) && value.HasFlag(NotAnnotatedStatus_BRANDON.Private) ||
                isAssembly                         && value.HasFlag(NotAnnotatedStatus_BRANDON.Internal)) {
                return true;
            }
        ...
        private NotAnnotatedStatus_BRANDON PopulateAnnotationInfo(IList<CustomAttributeData> customAttributes) {
            foreach (CustomAttributeData attribute in customAttributes) {
                if (attribute is { AttributeType: { Name: "NullablePublicOnlyAttribute", Namespace: CompilerServicesNameSpace }, ConstructorArguments.Count: 1 }) {
                    if (attribute.ConstructorArguments[0].Value is bool and true) {
                        return NotAnnotatedStatus_BRANDON.Internal | NotAnnotatedStatus_BRANDON.Private;
                    }
                    else {
                        return NotAnnotatedStatus_BRANDON.Private;
                    }
                }
            }

            return NotAnnotatedStatus_BRANDON.None;
        }
        ...
        private static bool ParseNullableState(IList<CustomAttributeData> customAttributes, int index, ref NullabilityState state) {
            foreach (CustomAttributeData attribute in customAttributes) {
                if (attribute is { AttributeType: { Name: "NullableAttribute", Namespace: CompilerServicesNameSpace }, ConstructorArguments.Count: 1 }) {
    ...
    internal static class BRANDON {
        public static Exception Hubris([CallerMemberName] string? method = default) {
            throw new Exception($"Failed in Brandon's naive attempt to steal {method} from the gods");
        }

        // Apparently the actual metadata comparison is dependent on the runtime(?).
        // I don't care about that, so we're good to just use the MetadataToken.
        // See: https://source.dot.net/#Microsoft.CSharp/Microsoft/CSharp/RuntimeBinder/RuntimeBinderExtensions.cs,7cf948f0785674c9
        public static bool HasSameMetadataDefinitionAs(this MemberInfo mi1, MemberInfo mi2) {
            return mi1.Module.Equals(mi2.Module) && mi1.MetadataToken == mi2.MetadataToken;
        }

        public static MemberInfo GetMemberWithSameMetadataDefinitionAs(this Type owner, MemberInfo member) {
            const BindingFlags all = BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static | BindingFlags.Instance;
            foreach (MemberInfo myMemberInfo in owner.GetMembers(all)) {
                if (HasSameMetadataDefinitionAs(myMemberInfo, member)) {
                    return myMemberInfo;
                }
            }

            throw BRANDON.Hubris();
        }
    }
}
#endif