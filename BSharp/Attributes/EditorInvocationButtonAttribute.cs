...
namespace FowlFever.BSharp.Attributes {
    ...
    /// <li><see cref="ValidateTarget"/> will throw an exception if the given <c>"MonoBehaviourEditor"</c> is invalid - which in this case would be anything that contains parameters.</li>
    /// </remarks>
    [MeansImplicitUse]
    [PublicAPI]
    [AttributeUsage(AttributeTargets.Method)]
    public class EditorInvocationButtonAttribute : BrandonAttribute {
        public override void ValidateTarget(MemberInfo target) {
            ...
            throw new InvalidAttributeTargetException<EditorInvocationButtonAttribute>(
                $"Methods annotated with {GetType().Name} must have exactly 0 parameters, but {target.Name} has {arity}!",
                target
            );
        }
    }
