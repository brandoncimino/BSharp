...
namespace FowlFever.BSharp {
    ...
    public static partial class Mathb {
        ...
        [Pure] public static short Clamp(this short value, short min, short max) => value <= min ? min : value >= max ? max : value;
        [Pure] public static int     Clamp(this int     value, int     min, int     max) => value <= min ? min : value >= max ? max : value;
        [Pure] public static long    Clamp(this long    value, long    min, long    max) => value <= min ? min : value >= max ? max : value;
        [Pure] public static float   Clamp(this float   value, float   min, float   max) => value <= min ? min : value >= max ? max : value;
        [Pure] public static double  Clamp(this double  value, double  min, double  max) => value <= min ? min : value >= max ? max : value;
        [Pure] public static decimal Clamp(this decimal value, decimal min, decimal max) => value <= min ? min : value >= max ? max : value;
        [Pure] public static ushort  Clamp(this ushort  value, ushort  min, ushort  max) => value <= min ? min : value >= max ? max : value;
        [Pure] public static uint    Clamp(this uint    value, uint    min, uint    max) => value <= min ? min : value >= max ? max : value;
        [Pure] public static ulong   Clamp(this ulong   value, ulong   min, ulong   max) => value <= min ? min : value >= max ? max : value;
        [Pure] public static byte    Clamp(this byte    value, byte    min, byte    max) => value <= min ? min : value >= max ? max : value;
        [Pure] public static sbyte   Clamp(this sbyte   value, sbyte   min, sbyte   max) => value <= min ? min : value >= max ? max : value;
        ...
        [Pure] public static double  Round(this float   value, RoundingDirection direction)                       => Rounder.Get(direction).Round(value);
        [Pure] public static double  Round(this double  value, RoundingDirection direction)                       => Rounder.Get(direction).Round(value);
        [Pure] public static decimal Round(this decimal value, RoundingDirection direction)                       => Rounder.Get(direction).Round(value);
        ...
        [Pure] public static double  Round(this float   value, int               digits,   RoundingDirection direction) => value.Round(direction).ToInt();
        [Pure] public static double  Round(this double  value, int               digits,   RoundingDirection direction) => value.Round(direction).ToInt();
        [Pure] public static decimal Round(this decimal value, int               decimals, RoundingDirection direction) => value.Round(direction).ToInt();
        ...
        [Pure] public static int     RoundToInt(this float   value, RoundingDirection direction)                                  => Rounder.Get(direction).RoundToInt(value);
        [Pure] public static int     RoundToInt(this double  value, RoundingDirection direction)                                  => Rounder.Get(direction).RoundToInt(value);
        [Pure] public static int     RoundToInt(this decimal value, RoundingDirection direction)                                  => Rounder.Get(direction).RoundToInt(value);
        ...
        [Pure] public static double  RoundToInt(this float   value, int              digits,   RoundingDirection direction) => Rounder.Get(direction).RoundToInt(value);
        [Pure] public static double  RoundToInt(this double  value, int              digits,   RoundingDirection direction) => Rounder.Get(direction).RoundToInt(value);
        [Pure] public static decimal RoundToInt(this decimal value, int              decimals, RoundingDirection direction) => Rounder.Get(direction).RoundToInt(value);
