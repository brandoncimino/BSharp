...
/// <summary>
/// Separates a set of <see cref="Indexes"/> into individual <see cref="Range"/>s that are proportional to the given <see cref="Weights"/>.
/// </summary>
    ...
    protected override IEnumerable<Range> GetPortions() {
        ...
        foreach (var w in Weights) {
            if (i == weightCount - 1) {
                ...
                var splerp       = amountLeft.Splerp(nextFraction);
                var soFar        = Source - amountLeft;

                yield return soFar..(soFar + splerp.A);
                amountLeft = splerp.B;
            }
