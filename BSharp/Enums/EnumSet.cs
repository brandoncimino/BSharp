...
namespace FowlFever.BSharp.Enums {
    ...
    public static class EnumSet {
        public static EnumSet<T> Of<T>(params T[] enumValues)
            where T : struct, Enum {
            return new EnumSet<T>(enumValues);
        }

        public static EnumSet<T> Inverted<T>(this ISet<T> original)
            where T : struct, Enum {
            return new EnumSet<T>(original);
        }

        public static EnumSet<T> Of<T>(IEnumerable<T> values)
            where T : struct, Enum {
            return new EnumSet<T>(values);
        }

        public static EnumSet<T> OfAllValues<T>()
            where T : struct, Enum {
        ...
        public static EnumSet<T> Of<T>(params IEnumerable<T>[] sets)
            where T : struct, Enum {
