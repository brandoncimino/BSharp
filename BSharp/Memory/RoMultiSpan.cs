...
/// </summary>
/// <remarks>
/// This "collection" has semantics similar to <see cref="System.Collections.Immutable">System.Collections.Immutable</see>, and so methods like <see cref="Add"/> will return a <b>new</b> <see cref="RoMultiSpan{T}"/>.
/// </remarks>
...
public readonly ref partial struct RoMultiSpan<T> {
    ...
    public RoMultiSpan<T> Add(ReadOnlySpan<T> newSpan) {
        if (SpanCount >= MaxSpans) {
            throw new InvalidOperationException($"Cannot add a new {newSpan.GetTypeName()} to this {GetType().GetTypeName()}: it already contains {MaxSpans} spans!");
        }
    ...
    public RoMultiSpan<T> AddRange(RoMultiSpan<T> newSpans) {
        Must.Have(SpanCount + newSpans.SpanCount <= MaxSpans);
        var result = this;
    ...
    internal string Describe() {
        ...
        sb.AppendLine(new string(Chars.EmDash, typeName.VisibleLength()));

        for (int i = 0; i < MaxSpans; i++) {
            if (TryGetSpan(i, out var span)) {
                sb.AppendLine($"[{i}] {span.ToArray().JoinString(", ", "[", "]")}");
            }
            else {
                sb.AppendLine($"[{i}] {Ansi.Italic.On}{Ansi.Gray.Fg}not used{Ansi.Reset.All}");
            }
        }
