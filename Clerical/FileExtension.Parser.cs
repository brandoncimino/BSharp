...
public readonly partial struct FileExtension {
    [UsedImplicitly]
    internal class Parser : IClericalParser<FileExtension, bool> {
        ...
        public static FileExtension Parse_Internal(SpanOrSegment s, bool strict) {
            var success = TryParse_Internal(s, strict, true, out var result);
            Debug.Assert(success, "Failures should have already thrown an exception!");
            return result;
        }
        ...
        public static FileExtension Parse_Forgiving(SpanOrSegment s) {
            return Parse_Internal(s, false);
        }
        ...
        public static FileExtension Parse_Strict(SpanOrSegment s) {
            return Parse_Internal(s, true);
        }
        ...
        [Pure]
        public static bool TryParse_Internal(SpanOrSegment input, bool strict, bool throwOnFailure, out FileExtension result) {
            if (strict is false) {
                input = input.Trim();
            }
        ...
        public static ValidationResult ValidateChar(char c) {
    ...
    private static void DebugAssert_PerfectExtension(ReadOnlySpan<char> s) {
        ...
        foreach (var c in afterPeriod) {
            Debug.Assert(Parser.ValidateChar(c) is Parser.ValidationResult.Perfect, $"Bad char: {c}");
        }
    }
}