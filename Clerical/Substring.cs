...
internal readonly record struct Substring([field: MaybeNull] string Source, int Start, int Length) {
    ...
    [Pure] public static                   Substring AllOf(string       str)       => new(str, 0, str.Length);
    [Pure] public static implicit operator Substring(string             str)       => AllOf(str);
    [Pure] public static implicit operator ReadOnlySpan<char>(Substring substring) => substring.AsSpan();
